...
class HabitTracker:
    ...
    def record_today(self, user_id: str):
        ...
        # 分も含めた時刻（グラフがきれいになる）
        completion_hour = now.hour + now.minute / 60

        self.data_manager.save_click_log(
            user_id, log_date, completion_hour
        )
    ...
    def needs_reset(self, logs: list, threshold: int) -> bool:
        ...
        # 最新ログ日を取得（安全）
        sorted_logs = sorted(logs, key=lambda x: x["log_date"], reverse=True)
        last_date_str = sorted_logs[0]["log_date"]

        last_date_obj = datetime.datetime.strptime(
            last_date_str, DATE_FORMAT
        ).date()
    ...
    # ------------------ ユーティリティ ------------------
    @staticmethod
    def hour_to_hhmm(hour_float: float) -> str:
        """float時間 → HH:MM 文字列"""
